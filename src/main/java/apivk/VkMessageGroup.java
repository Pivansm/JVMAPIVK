...
public class VkMessageGroup {
    private VkApiClient vk;
    private static int ts;
    private GroupActor actor;
    private static int maxMsgId = -1;

    public VkMessageGroup(String acc_tokn) throws ClientException, ApiException {
        ...
        try {
            ...
            groupId = 151897652;
            access_token = acc_tokn;
            actor = new GroupActor(groupId, access_token);
            //ts = vk.messages().getLongPollServer(actor).execute().getTs();
            //
            //ts = vk.messages().get(actor).execute();
            ts = 1;
        } catch (IOException e) {
            e.printStackTrace();
